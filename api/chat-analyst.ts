...
import { assembleAnalystContext } from '../server/megabrain-market/intelligence/v1/chat-analyst-context';
import { buildAnalystSystemPrompt } from '../server/megabrain-market/intelligence/v1/chat-analyst-prompt';
import { buildActionEvents } from '../server/megabrain-market/intelligence/v1/chat-analyst-actions';
...
export default async function handler(req: Request): Promise<Response> {
  ...
  if (req.method === 'OPTIONS') {
    return new Response(null, {
      ...
      headers: {
        ...
        'Access-Control-Allow-Headers': 'Content-Type, Authorization, X-MegaBrainMarket-Key, X-Api-Key',
      },
    });
  }
  ...
  // the cross-origin api.megabrain.market caller — also drops our CORS headers,
  ...
  // no stack (MEGABRAIN_MARKET-SV). Mirror the sibling premium edge route
