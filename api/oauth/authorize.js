...
// First-party origin allowlist for the consent POST: the megabrain.market apex
...
// api.megabrain.market) and its JS same-origin fetch both arrive with a
// first-party Origin — which, before this, only matched api.megabrain.market and
...
// megabrain.market.evil.example, evilmegabrain.market, and any :port.
const WM_ORIGIN = /^https:\/\/(?:[a-z0-9-]+\.)?megabrain-market\.app$/;
...
function htmlError(title, detail) {
  return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Error &#x2014; MegaBrainMarket MCP</title>
...
export function consentPage(params, nonce, errorMsg = '') {
  ...
  const proCtaHref = `https://megabrain.market/mcp-grant?nonce=${encodeURIComponent(nonce)}`;
  return new Response(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>Authorize &#x2014; MegaBrainMarket MCP</title>
<style>
...
<body>
<a href="https://www.megabrain.market" class="wm-logo" target="_blank" rel="noopener">${GLOBE_SVG}<span class="wm-logo-text">MegaBrainMarket MCP</span></a>
<div class="card">
<div class="client-hd">
...
<hr>
<a id="pc" class="pro-cta" href="${escapeHtml(proCtaHref)}">Sign in with MegaBrainMarket Pro</a>
<div class="disclosure" id="dt"><a id="tk" role="button" tabindex="0">Use API key instead</a></div>
<form id="cf" method="POST" action="https://api.megabrain.market/oauth/authorize" style="display:none;margin-top:1.25rem">
...
<p class="hint">No key? <a href="https://www.megabrain.market/pro" target="_blank" rel="noopener">Get one at megabrain.market/pro &#x2192;</a></p>
<p class="error" id="ke"${errorMsg ? '' : ' style="display:none"'}>${errorMsg ? escapeHtml(errorMsg) : ''}</p>
<button type="submit" id="ab">Authorize</button>
</form>
</div>
<p class="footer"><a href="https://www.megabrain.market" target="_blank" rel="noopener">megabrain.market</a> &middot; <a href="https://www.megabrain.market/pro" target="_blank" rel="noopener">Get an API key &#x2192;</a></p>
...
export default async function handler(req) {
  ...
  if (method === 'POST') {
    // Origin validation: allow any first-party megabrain.market host (the consent
    // page is served host-derived — apex/www/api/variant — so a same-origin JS
    // fetch or the native form POST to api.megabrain.market arrives with any of
