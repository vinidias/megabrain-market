...
function downloadStory(): void {
  ...
  a.download = `megabrain-market-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-megabrain-market.png`, { type: 'image/png' });
