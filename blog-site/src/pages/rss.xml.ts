...
function getEnclosure(heroImage: string | undefined) {
  ...
  const url = /^https?:\/\//.test(heroImage)
    ? heroImage
    : `https://www.megabrain.market${heroImage}`;
...
export async function GET(context: { site: URL }) {
  ...
  return rss({
    ...
    customData: [
      ...
      `<atom:link href="https://www.megabrain.market/blog/rss.xml" rel="self" type="application/rss+xml" />`,
    ].join(''),
    items: posts
