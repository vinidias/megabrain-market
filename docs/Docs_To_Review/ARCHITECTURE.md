...
| **IndexedDB** | `megabrain-market_db` | Client-side storage for playback snapshots and temporal baseline data. |
...
MegaBrain Market ships as three product variants from a single codebase. Each variant surfaces a different subset of panels, map layers, and data sources.

| Variant | Domain | Focus |
|---|---|---|
| `full` | megabrain.market | Geopolitics, military, OSINT, conflicts, markets |
| `tech` | tech.megabrain.market | AI/ML, startups, cybersecurity, developer tools |
| `finance` | finance.megabrain.market | Markets, trading, central banks, macro indicators |
...
```
localStorage('megabrain-market-variant')  →  import.meta.env.VITE_VARIANT  →  default 'full'
```
...
export const SITE_VARIANT: string = (() => {
  if (typeof window !== 'undefined') {
    const stored = localStorage.getItem('megabrain-market-variant');
...
graph TD
    ...
    subgraph LayerConfig["Layer Configuration"]
        ...
        UserPref["localStorage overrides<br/>(megabrain-market-layers)"]
...
2. **User localStorage** — Stored under the key `megabrain-market-layers`. Users toggle layers in the map controls UI, and their preferences persist across sessions.
...
MegaBrain Market employs a five-tier caching strategy to minimise API costs, reduce latency, and enable offline operation.
...
graph TD
    ...
    subgraph Tier5["Tier 5: Persistent Cache"]
        ...
        LSFallback["localStorage fallback<br/>prefix: megabrain-market-persistent-cache:"]
        PC --> TauriInvoke
        PC --> LSFallback
    end
...
The `megabrain-market_db` IndexedDB database contains two object stores:
...
**2. Feature Toggles** — 14 `RuntimeFeatureId` values stored in localStorage under the key `megabrain-market-runtime-feature-toggles`:
...
MegaBrain Market runs machine-learning inference directly in the browser using ONNX Runtime Web via Transformers.js, with API-based fallbacks for constrained devices.
...
MegaBrain Market uses a circuit-breaker pattern to manage transient failures across its many data sources, preventing cascade failures and providing graceful degradation.
