...
MegaBrain Market is an AI-powered real-time global intelligence dashboard built with **vanilla TypeScript** — no framework, no reactive stores. All state is managed manually through class properties, `localStorage`, `IndexedDB`, and URL query parameters.
...
Stored in `localStorage` under `megabrain-market-panels` as `Record<string, PanelConfig>`. Includes per-panel `enabled` state, `name`, and `priority`. Controlled by the variant config and user overrides.
...
| Key | Type | Purpose |
|-----|------|---------|
| `megabrain-market-panels` | `Record<string, PanelConfig>` | Per-panel enabled/name/priority |
| `megabrain-market-monitors` | `Monitor[]` | Color/label configs for monitors |
| `megabrain-market-layers` | `MapLayers` | Enabled/disabled map layer toggles |
| `megabrain-market-disabled-feeds` | `string[]` | User-disabled news feed sources |
...
sequenceDiagram
    ...
    setTheme->>localStorage: setItem('megabrain-market-theme', 'light')
...
- **Database name:** `megabrain-market_db`
- **Version:** `1`
...
export async function initDB(): Promise<IDBDatabase> {
  // Opens or creates megabrain-market_db v1
...
- **Toggles key:** `megabrain-market-runtime-feature-toggles` in `localStorage`
...
Complete reference of every `localStorage` key used by MegaBrain Market:

| Key | Purpose | Format | Source |
|-----|---------|--------|--------|
| `megabrain-market-variant` | Active variant override | `'full' \| 'tech' \| 'finance'` | `src/config/variant.ts`, `App.ts` |
| `megabrain-market-theme` | Theme preference | `'dark' \| 'light'` | `src/utils/theme-manager.ts` |
| `panel-order` | Panel arrangement order | `string[]` (JSON) | `App.ts` |
| `megabrain-market-panel-spans` | Panel grid sizes | `Record<string, number>` (JSON) | `src/components/Panel.ts` |
| `megabrain-market-panels` | Panel enabled/config state | `Record<string, PanelConfig>` (JSON) | `STORAGE_KEYS.panels` |
| `megabrain-market-monitors` | Monitor color/label configs | `Monitor[]` (JSON) | `STORAGE_KEYS.monitors` |
| `megabrain-market-layers` | Map layer toggles | `MapLayers` (JSON) | `STORAGE_KEYS.mapLayers` |
| `megabrain-market-disabled-feeds` | User-disabled news sources | `string[]` (JSON) | `STORAGE_KEYS.disabledFeeds` |
| `megabrain-market-runtime-feature-toggles` | Desktop feature toggles | `Record<RuntimeFeatureId, boolean>` (JSON) | `src/services/runtime-config.ts` |
| `megabrain-market-persistent-cache:{key}` | Persistent data cache entries | `CacheEnvelope<T>` (JSON) | `src/services/persistent-cache.ts` |
| `wm-update-dismissed-{version}` | Dismissed update notifications | `'1'` | `App.ts` |
| `megabrain-market-panel-order-v1.9` | Panel order migration flag | `'done'` | `App.ts` (one-time migration) |
| `megabrain-market-tech-insights-top-v1` | Tech variant migration flag | `'done'` | `App.ts` (one-time migration) |
...
flowchart TB
    subgraph "Persistent Storage"
        LS["localStorage"]
        IDB["IndexedDB (megabrain-market_db)"]
