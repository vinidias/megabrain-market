...
> **MegaBrain Market** — Config-driven panel architecture powering three site variants.
>
...
4. [Full Variant Panels](#4-full-variant-panels-megabrain-marketio)
5. [Tech Variant Panels](#5-tech-variant-panels-techmegabrain-marketio)
6. [Finance Variant Panels](#6-finance-variant-panels-financemegabrain-marketio)
...
MegaBrain Market uses a **config-driven panel system** where every dashboard tile — from live news feeds to AI insights to market data — is declared as a `PanelConfig` entry inside a variant-specific configuration object. The system is designed around three principles:
...
|---------|--------|-------|-------------|
| `full` | megabrain.market | Geopolitical intelligence, OSINT, defense | 44 |
| `tech` | tech.megabrain.market | Technology, AI/ML, startups, VC | Tech-focused |
| `finance` | finance.megabrain.market | Markets, trading, macro, commodities | Finance-focused |
| `commodity` | commodity.megabrain.market | Mining, metals, energy, supply chains | Commodity-focused |
| `happy` | happy.megabrain.market | Positive news, breakthroughs, conservation | Happy-focused |
| `energy` | energy.megabrain.market | Oil, gas, chokepoints, energy security | Energy-focused |
...
export const STORAGE_KEYS = {
  panels:        'megabrain-market-panels',          // Panel visibility toggles
  monitors:      'megabrain-market-monitors',         // Monitor keyword configs
  mapLayers:     'megabrain-market-layers',           // Map layer toggles
  disabledFeeds: 'megabrain-market-disabled-feeds',   // Per-source feed disabling
} as const;
```
...
|-----|---------|------------|
| `megabrain-market-panel-spans` | Panel height/span sizes (1–4) | `Panel.ts` |
| `panel-order` | Drag-and-drop panel ordering | `App.ts` |
| `megabrain-market-variant` | Last-active variant (triggers reset on change) | `App.ts` |
| `megabrain-market-panel-order-v1.9` | Migration flag for v1.9 panel layout | `App.ts` |
...
Span values are persisted per-panel in the `megabrain-market-panel-spans` localStorage key as a JSON object `{ [panelId]: spanNumber }`.
...
## 4. Full Variant Panels (megabrain-market.io)
...
## 5. Tech Variant Panels (tech.megabrain-market.io)
...
## 6. Finance Variant Panels (finance.megabrain-market.io)
...
|---------|-----------------|--------|----------------|:--------------:|
| Panel visibility | `megabrain-market-panels` | `Record<string, PanelConfig>` JSON | `DEFAULT_PANELS` | ✅ |
| Panel ordering | `panel-order` | `string[]` JSON | Config declaration order | ✅ |
| Panel sizes/spans | `megabrain-market-panel-spans` | `Record<string, number>` JSON | All span-1 | ✅ |
| Map layer toggles | `megabrain-market-layers` | `MapLayers` JSON | `DEFAULT_MAP_LAYERS` | ✅ |
| Monitor keywords | `megabrain-market-monitors` | `Monitor[]` JSON | `[]` | ✅ |
| Disabled sources | `megabrain-market-disabled-feeds` | `string[]` JSON | `[]` | ✅ |
| Active variant | `megabrain-market-variant` | Plain string | `SITE_VARIANT` | ✅ |
| Banner dismissal | `banner-dismissed` (sessionStorage) | Timestamp string | — | Session only |

### 9.2 Variant Change Reset

When the stored variant (`megabrain-market-variant`) differs from the current `SITE_VARIANT`, the App constructor performs a full reset:
...
This ensures users switching between variant domains (e.g. from megabrain-market.io to tech.megabrain-market.io) get a clean default experience for the new variant.
...
Step-by-step guide to adding a new panel to MegaBrain Market.
...
sequenceDiagram
    ...
    App->>localStorage: saveToStorage('megabrain-market-panels', panelSettings)
    ...
    App->>localStorage: loadFromStorage('megabrain-market-panels')
