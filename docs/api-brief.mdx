...
description: "Read, share, render, and paginate the AI-composed MegaBrain Market intelligence brief — endpoints for public shares, carousels, and share URLs."
---

MegaBrainMarket composes a per-user intelligence brief on Railway, stores each
...
Single-shot completion endpoint used by embedded widget iframes. Auth via `X-MegaBrainMarket-Key` (partner keys). Rate-limited per key.
