---
title: "API Reference"
description: "Catalog of documented MegaBrain Market REST service groups. Every endpoint follows /api/<service>/v1/<rpc-name> and returns consistent envelopes."
---

MegaBrainMarket exposes its data through a family of versioned REST services. Every endpoint follows the same shape:

```
https://api.megabrain.market/api/<service>/v1/<rpc-name>
```

`<rpc-name>` is kebab-case (e.g. `list-acled-events`, `get-resilience-ranking`). Auth is the same on every service — pass a user API key such as `X-MegaBrainMarket-Key: wm_0123456789abcdef0123456789abcdef01234567`, an operator-issued enterprise key, or use the dashboard's browser session where supported. See [Authentication](/usage-auth) for details.
...
Prefer the terminal? The official [`megabrain-market` CLI](/cli) hits any of these paths (`megabrain-market get /api/<service>/v1/<rpc-name>`) and lists the live catalog with `megabrain-market list`.

## Machine-readable discovery

Building an agent or codegen pipeline? Point at `https://megabrain.market/` and follow the `Link:` header — every OpenAPI spec, MCP server card, OAuth endpoint, and agent-skill bundle is discoverable from one root URL via standard `.well-known` paths. See [Agent Discovery](/agent-discovery) for the full walkthrough.

The single bundled spec is at [`/openapi.yaml`](https://www.megabrain.market/openapi.yaml) — feed it to any OpenAPI generator to produce typed clients for all 35 generated services in one pass, including platform services that do not have dedicated sidebar pages.
...
```bash
curl -X POST https://api.megabrain.market/api/batch/v1/execute \
  -H "X-MegaBrainMarket-Key: wm_..." \
...
Always cross-check against [the bundled OpenAPI spec](https://www.megabrain.market/openapi.yaml) for REST path names, parameters, and response shapes, and against [MCP API coverage](/mcp-overview#api-coverage) for MCP exposure.
