---
title: "CORS"
description: "How cross-origin request protection works in MegaBrain Market and what to do when adding new API endpoints — allowlists, preflights, and Worker gate."
---
...
| Pattern | Matches |
| --- | --- |
| `(*.)?megabrain.market` | Production + subdomains (`tech.`, `finance.`, etc.) |
| `megabrain-market-*-elie-*.vercel.app` | Vercel preview deploys |
...
- `Content-Type`
- `Authorization`
- `X-MegaBrainMarket-Key` (API key for desktop/third-party access). See [API Key Gating](/api-key-deployment) for key management details.
- `X-Api-Key`
- `X-Widget-Key`
- `X-Pro-Key`
