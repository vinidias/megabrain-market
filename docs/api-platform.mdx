...
description: "Bootstrap, health, version, cache-purge, and user-preference endpoints — the plumbing every MegaBrain Market client talks to on startup and cleanup."
---
...
- **Auth**: browser `wm-session` cookie, `X-MegaBrainMarket-Key`, or the `X-Api-Key` alias. User-issued keys are validated for current API access.
- **Anonymous weather**: `?keys=weatherAlerts` is public **only when no API key header is sent**. If you attach `X-MegaBrainMarket-Key` / `X-Api-Key`, the request is fully validated even for weather — a malformed key returns `401`, a key without current API access returns `403`. Callers that always send a key must send a valid, entitled key (or omit the header to use the anonymous weather path).
- **Server-to-server**: call `https://api.megabrain.market/api/bootstrap` directly with `X-MegaBrainMarket-Key: wm_...`. There is no separate gateway host, token exchange, activation step, or IP allow-list requirement for this endpoint.
...
Returns the latest **GitHub Release** of `vinidias/megabrain-market`. Used by the desktop app to detect a newer published release and prompt the user to update. It is **not** the currently-deployed Vercel commit.
...
Redirects to the matching asset on the latest GitHub release of `vinidias/megabrain-market`. Returns `302` to the asset URL on success, or `302` to [releases/latest](https://github.com/vinidias/megabrain-market/releases/latest) on any failure (unknown platform, no match, GitHub error).
...
- `X-MegaBrainMarket-Desktop-Timestamp`: Unix epoch milliseconds, within 5 minutes of server time.
