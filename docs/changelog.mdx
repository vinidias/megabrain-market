---
title: "Changelog"
description: "All notable changes to MegaBrain Market organized by release — new features, bug fixes, API additions, and breaking changes across every version."
rss: true
---

All notable changes to MegaBrain Market are documented here. Subscribe via [RSS](https://www.megabrain.market/docs/changelog/rss.xml) to stay updated.
...
- **Sebuf API migration (#3207)** — scenario + supply-chain endpoints moved to the typed sebuf contract. RPC URLs now derive from method names; five renamed v1 URLs remain live as thin aliases so existing integrations keep working:
  ...
  Aliases retire at the next v1→v2 break (tracked in [#3282](https://github.com/vinidias/megabrain-market/issues/3282)).
...
- **CDN cache bypass closed**: `CDN-Cache-Control` header now only emitted for trusted origins (megabrain.market, Vercel previews, Tauri). No-origin server-side requests always invoke the edge function so `validateApiKey` runs, preventing a cached 200 from being served to external scrapers.
