---
title: "MCP Apps: interactive ui:// widgets for MegaBrainMarket"
description: "Complete contract for MegaBrainMarket's MCP Apps interactive ui:// resources, tool links, host flow, security posture, and drift checks."
---

MegaBrainMarket supports [MCP Apps](https://modelcontextprotocol.io/extensions/apps/build) through the `io.modelcontextprotocol/ui` extension. The current fleet ships 10 MCP Apps: self-contained `ui://` HTML resources that an MCP Apps host can render inline after a linked tool call.
...
| Transport endpoint | `https://megabrain.market/mcp` |
| UI resource scheme | `ui://megabrain-market/...` |
...
|---|---|---|---|
| `ui://megabrain-market/country-risk.html` | `get_country_risk` | Country Risk (interactive) | Composite Instability Index score, component breakdown, travel advisory, and sanctions exposure. |
| `ui://megabrain-market/world-brief.html` | `get_world_brief` | World Brief (interactive) | AI-summarised global brief, grounding headlines, and source feed articles. |
| `ui://megabrain-market/country-brief.html` | `get_country_brief` | Country Brief (interactive) | Per-country intelligence brief, analytical framework lens, and grounding sources. |
| `ui://megabrain-market/market-radar.html` | `get_market_data` | Market Radar (interactive) | Fear & Greed composite plus equity, commodity, crypto, Gulf, and sector quote tables with signed, colour-coded change. |
| `ui://megabrain-market/chokepoint-monitor.html` | `get_chokepoint_status` | Chokepoint Monitor (interactive) | Per-chokepoint transit summaries, week-over-week change, tanker split, and risk-level badge. |
| `ui://megabrain-market/news-intelligence.html` | `get_news_intelligence` | News Intelligence (interactive) | AI-classified top stories with category, alert flag, country, and source. |
| `ui://megabrain-market/conflict-events.html` | `get_conflict_events` | Conflict Events (interactive) | Active armed-conflict events (belligerents, violence type, country, fatalities, date) from UCDP. |
| `ui://megabrain-market/natural-disasters.html` | `get_natural_disasters` | Natural Disasters (interactive) | Recent earthquakes (USGS magnitude, place, time) and active wildfires (NASA FIRMS), grouped. |
| `ui://megabrain-market/prediction-markets.html` | `get_prediction_markets` | Prediction Markets (interactive) | Event-contract odds grouped by category (geopolitical, tech, finance) with a probability bar per market. |
| `ui://megabrain-market/forecasts.html` | `get_forecast_predictions` | Forecasts (interactive) | AI-generated geopolitical and economic forecasts as probability cards (title, domain, region). |

## Runtime Flow

1. The host calls `initialize` on `https://megabrain.market/mcp`.
2. MegaBrainMarket returns the normal MCP capabilities plus `capabilities.extensions["io.modelcontextprotocol/ui"]`.
...
The view never fetches live MegaBrainMarket data itself. Live data always reaches the app through the host after a normal tool call.
...
|---|---|---|---|---|
| UI template | `resources/read` `ui://megabrain-market/market-radar.html` | No | No | Static HTML shell, no data and no upstream fetch. |
| Public metadata | `resources/read` `megabrain-market://seed-meta/freshness` | No | No | Metadata-only health/freshness probe. |
| Data template instantiation | `resources/read` `megabrain-market://countries/de/risk` | Yes | Yes | Routes through the same dispatcher as the equivalent `tools/call`. |
