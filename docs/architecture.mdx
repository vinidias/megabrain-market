...
description: "Design principles, intelligence tradecraft, and algorithmic decisions behind MegaBrain Market — how the map-first dashboard is engineered and why."
---

> **Looking for the system reference?** See [`ARCHITECTURE.md`](https://github.com/vinidias/megabrain-market/blob/main/ARCHITECTURE.md) for deployment topology, directory layout, caching layers, CI/CD, and contributor how-to guides.
...
MegaBrain Market is written in vanilla TypeScript — no frontend framework (React, Vue, Svelte, Angular) is used. This is a deliberate architectural decision, not an oversight.
...
All six dashboard variants (MegaBrain Market, Tech Monitor, Finance Monitor, Commodity Monitor, Happy Monitor, Energy Monitor) serve from a **single Vercel deployment**. The variant is determined at runtime by hostname detection:
...
| `megabrain.market` (default) | `full` |

On the desktop app, the variant is stored in `localStorage['megabrain-market-variant']` and can be switched without rebuilding. The variant selector in the header bar navigates between deployed domains on the web or toggles the localStorage value on desktop.
...
Sentry captures unhandled exceptions and promise rejections in production, with environment-aware routing (production on `megabrain.market`, preview on `*.vercel.app`, disabled on localhost and Tauri desktop).
...
Transactions are sampled at 10% to balance observability with cost. Release tracking (`megabrain-market@{version}`) enables regression detection across deployments.
...
The Service Status panel tracks the operational health of external services that MegaBrainMarket users may depend on.
