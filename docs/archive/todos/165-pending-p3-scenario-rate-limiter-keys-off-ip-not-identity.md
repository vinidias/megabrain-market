...
- See MEMORY: `feedback_is_caller_premium_trusted_origin.md` — the API key is extractable from `X-MegaBrainMarket-Key`
...
```ts
const apiKey = req.headers.get('x-megabrain-market-key');
...
## Acceptance Criteria
- [ ] Rate limit key uses `X-MegaBrainMarket-Key` when present
