...
description: "Every error shape the MegaBrain Market MCP server emits — JSON-RPC codes, HTTP statuses, soft-behavior envelopes, and recommended client responses."
---
...
- **Both 401 paths set `WWW-Authenticate`** with `realm="megabrain-market"` and a `resource_metadata` pointer at `/.well-known/oauth-protected-resource`. RFC 9728-aware clients (Claude Desktop, MCP Inspector) bounce through the OAuth flow on this header without further intervention.

## JSON-RPC error codes

| Code      | Meaning (this server)                                                  | Paired HTTP status        | Recovery                                                              |
|-----------|------------------------------------------------------------------------|---------------------------|-----------------------------------------------------------------------|
| `-32001`  | Unauthenticated, invalid token, or subscription not active             | **401**                   | Re-authenticate via OAuth or fix the `X-MegaBrainMarket-Key` header      |
...
1. **No `Authorization` bearer AND no `X-MegaBrainMarket-Key`** — the client called `/mcp` with no credentials.
...
3. **`X-MegaBrainMarket-Key: <key>` but `<key>` isn't in the valid set** — the API key is wrong.
...
|-------------------------------------------|-------------------------------------------------------------------------------------|---------------------------------------------------------|
| `X-MegaBrainMarket-Key`                      | `Rate limit exceeded. Max 60 requests per minute per API key.`                      | `api/mcp/auth.ts` `applyPerMinuteLimit` API-key branch  |
