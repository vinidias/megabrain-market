...
description: "Script country briefs, risk scores, and any of the 39 MCP tools from your shell with the official megabrain-market npm CLI — no API integration to write."
---

[`megabrain-market`](https://www.npmjs.com/package/megabrain-market) is the official command-line client for the MegaBrain Market global-intelligence API. It's a thin, **dependency-free** wrapper over the [MCP server](/mcp-overview) (the recommended agent surface) with a REST escape hatch, so you can pull briefs, risk scores, and conflict / cyber / market / news feeds from a shell script or an agent without writing a single line of HTTP plumbing. It ships as ESM and runs on **Node 18+**.

The source lives in [`cli/`](https://github.com/vinidias/megabrain-market/tree/main/cli) in the main repository and is published to npm on every `cli-v*` release tag.
...
```sh
npm install -g megabrain-market    # installs the `megabrain-market` command (alias: `wm`)
...
Data commands map to MCP `tools/call` and require a user API key. Grab one at [megabrain.market/pro](https://www.megabrain.market/pro), then either pass it per-call or export it once:

```sh
export MEGABRAIN_MARKET_API_KEY=wm_xxxxxxxx
megabrain-market risk IR                       # reads the key from the environment
# — or —
megabrain-market risk IR --api-key wm_xxxxxxxx # pass it explicitly
```

The key is sent as the `X-MegaBrainMarket-Key` header. See [Authentication](/usage-auth) for how keys, OAuth, and browser sessions differ, and [Rate Limits](/usage-rate-limits) for the per-plan allowances. If you call a data command with no key, the CLI exits `1` and prints a hint telling you exactly which flag to pass.
...
```sh
megabrain-market country IR                              # AI strategic brief
megabrain-market conflicts --country IR --limit 5        # recent conflict events
megabrain-market markets --asset_class crypto            # crypto quotes
```
...
```sh
megabrain-market call get_cyber_threats --min_severity 7
megabrain-market call get_market_data --args '{"symbols":["AAPL","MSFT"]}'   # typed JSON args
megabrain-market prompts        # list pre-built workflow prompt templates
megabrain-market resources      # list addressable resource URIs
```
...
```sh
megabrain-market markets --jmespath 'data."stocks-bootstrap".quotes[?symbol==`AAPL`].{s:symbol,p:price}'
```
...
```sh
megabrain-market health --api-key wm_xxx      # API status / health check (needs a key)
megabrain-market get /api/cyber/v1/list-cyber-threats --api-key wm_xxx
megabrain-market list cyber                   # list documented REST operations from the live spec
```
...
| `--mcp-url <url>` | MCP endpoint (default `https://megabrain.market/mcp`) |
| `--base-url <url>` | REST base (default `https://api.megabrain.market`) |
