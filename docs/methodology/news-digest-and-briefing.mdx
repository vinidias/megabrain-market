...
description: "How MegaBrainMarket collects RSS/Atom feeds, classifies news, scores importance, tracks stories, filters briefs, uses LLMs, and controls bias."
---

## Scope

This page is the canonical public methodology for the news-feed digest,
daily digest notifications, and MegaBrainMarket Brief. It covers the automated
...
Feed inventory lives in `server/megabrain-market/news/v1/_feeds.ts`.
...
`energy` is a site and client-feed variant (`energy.megabrain.market`), but it
...
`server/megabrain-market/news/v1/_classifier.ts`.
...
MegaBrainMarket Brief uses two LLM surfaces:
...
- Feed inventory and digest build: `server/megabrain-market/news/v1/_feeds.ts`,
  `server/megabrain-market/news/v1/list-feed-digest.ts`
- Keyword classifier: `server/megabrain-market/news/v1/_classifier.ts`
- API contract: `proto/megabrain-market/news/v1/list_feed_digest.proto`
